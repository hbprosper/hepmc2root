...
class hepmc2pandas:
    ...
    def __str__(self, index):
        p = self.pbag
        d = " <%4d>" % (p["Particle_daughters"][index])
    ...
    def __call__(self):
        ...
        for line in inp:
            ...
            if key == "C":
                ...
                for line in inp:
                    token = str.split(line)
                    if int(token[11]) == vbarcode and token[0] == "P":
                        has_beam_particle = True
                        index = e["Event_numberP"][-1]
                        e["Event_numberP"][-1] += 1

                        p["Particle_x"].append(0)
                        p["Particle_y"].append(0)
                        p["Particle_z"].append(0)
                        ...
                        p["Particle_status"].append(int(token[8]))
                        self.pvertex[index] = int(token[11])
                        self.vertex[vbarcode].append(int(token[1]))
                    break
                if not has_beam_particle:
                    nout -= 1
            ...
            if len(self.vertex) >= e["Event_numberV"][-1]:
                for index in range(e["Event_numberP"][-1]):
                    ...
                    if code in self.vertex:
                        d = self.vertex[code]
                        p["Particle_daughters"][index] = d
                        # p["Particle_d2"][index] = d[1]
                    else:
                        p["Particle_daughters"][index] = []
